...
def process(img, old_ballots):
    ...
    try:
        new_img, person_boxes, object_boxes, original_boxes = detect_objects_yolo(new_img)
    except Exception as e:
        print(f"yolo ERROR, \n{e}")

#     try:
        # Modified to detect on cropped images and output [person bbox, face_name] to track corresponding face to body
    new_img, person_face_detected, old_ballots = detect_deepface_cropped(new_img, person_boxes, old_ballots, original_boxes)
#     except Exception as e:
